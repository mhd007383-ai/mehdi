...
const PantryModal: React.FC<PantryModalProps> = ({ 
  ...
  const [showOnlySpices, setShowOnlySpices] = useState(false);
  ...
  const filteredList = useMemo(() => {
    return list.filter(item => {
        const matchesSearch = searchTerm 
            ...
            : true;
        const matchesSpiceFilter = showOnlySpices ? item.isSpice : true;
        return matchesSearch && matchesSpiceFilter;
    });
  }, [list, searchTerm, showOnlySpices]);


  useEffect(() => {
    if (!isOpen) {
      setSearchTerm('');
      setShowOnlySpices(false);
    }
  }, [isOpen]);
        ...
        <div className="p-4 flex-grow overflow-y-auto">
          {isAnalyzing ? (
            ...
            <>
                 ...
                 <div className="mb-4 flex flex-col sm:flex-row items-center gap-4">
                    ...
                    <div className="flex-shrink-0">
                        <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 cursor-pointer">
                            <input
                                type="checkbox"
                                checked={showOnlySpices}
                                onChange={(e) => setShowOnlySpices(e.target.checked)}
              ...
              {list.length === 0 ? (
                  <p className="text-center text-gray-500 dark:text-gray-400 py-8">انبار شما خالی است.</p>
                  ...
                  <table className="w-full text-sm text-right text-gray-500 dark:text-gray-400">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" className="px-6 py-3">ادویه</th>
                    ...
                    <tbody>
                      {filteredList.map((item) => (
                        <tr key={item.name} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td className="px-6 py-4">{item.isSpice ? 'بله' : 'خیر'}</td>
